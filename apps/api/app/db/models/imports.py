...
goodreads_import_job_status_enum = ENUM(
    "queued",
    "running",
    "completed",
    "failed",
...
goodreads_import_row_result_enum = ENUM(
    "imported",
    "failed",
    "skipped",
...
class GoodreadsImportJob(Base):
    __tablename__ = "goodreads_import_jobs"
    __table_args__ = (
        sa.Index(
        ...
        sa.Index("ix_goodreads_import_jobs_status", "status"),
    )
    ...
    filename: Mapped[str] = mapped_column(sa.String(255), nullable=False)
    status: Mapped[str] = mapped_column(
        goodreads_import_job_status_enum, nullable=False
    )
    total_rows: Mapped[int] = mapped_column(
        sa.Integer, nullable=False, server_default="0"
    )
...
class GoodreadsImportJobRow(Base):
    __tablename__ = "goodreads_import_job_rows"
    __table_args__ = (
        sa.Index("ix_goodreads_import_job_rows_job_row", "job_id", "row_number"),
        sa.Index("ix_goodreads_import_job_rows_job_result", "job_id", "result"),
        sa.UniqueConstraint(
            "job_id",
            "identity_hash",
    ...
    uid: Mapped[str | None] = mapped_column(sa.String(255))
    result: Mapped[str] = mapped_column(
        goodreads_import_row_result_enum, nullable=False
    )
    message: Mapped[str] = mapped_column(sa.Text, nullable=False)
    work_id: Mapped[uuid.UUID | None] = mapped_column(sa.UUID(as_uuid=True))
    library_item_id: Mapped[uuid.UUID | None] = mapped_column(sa.UUID(as_uuid=True))
