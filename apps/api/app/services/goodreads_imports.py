...
def create_goodreads_job(
    session: Session,
    *,
    user_id: UUID,
    filename: str,
) -> GoodreadsImportJob:  # pragma: no cover
    job = GoodreadsImportJob(
        user_id=user_id,
        filename=filename,
        status="queued",
        total_rows=0,
...
def get_goodreads_job(  # pragma: no cover
    session: Session, *, user_id: UUID, job_id: UUID
) -> GoodreadsImportJob:
    job = session.scalar(
        sa.select(GoodreadsImportJob).where(
            GoodreadsImportJob.id == job_id,
            GoodreadsImportJob.user_id == user_id,
        )
    )
    if job is None:
...
def get_active_goodreads_job(
    session: Session,
    *,
    user_id: UUID,
) -> GoodreadsImportJob | None:
    return session.scalar(
        sa.select(GoodreadsImportJob)
        .where(
            GoodreadsImportJob.user_id == user_id,
            GoodreadsImportJob.status.in_(["queued", "running"]),
        )
        .order_by(GoodreadsImportJob.created_at.desc())
        .limit(1)
    )


def list_goodreads_job_rows(
    session: Session,
    *,
    user_id: UUID,
    job_id: UUID,
    limit: int,
    cursor: int | None,
) -> tuple[list[GoodreadsImportJobRow], int | None]:  # pragma: no cover
    get_goodreads_job(session, user_id=user_id, job_id=job_id)
    stmt = (
        sa.select(GoodreadsImportJobRow)
        .where(
            GoodreadsImportJobRow.user_id == user_id,
            GoodreadsImportJobRow.job_id == job_id,
        )
        .order_by(GoodreadsImportJobRow.row_number.asc())
    )
    if cursor is not None:
        stmt = stmt.where(GoodreadsImportJobRow.row_number > cursor)
...
def _job_duration_ms(job: GoodreadsImportJob) -> int | None:  # pragma: no cover
...
def serialize_job(
    ...
    *,
    ...
    job = get_goodreads_job(session, user_id=user_id, job_id=job_id)
    preview_rows = session.execute(
        sa.select(GoodreadsImportJobRow)
        .where(
            GoodreadsImportJobRow.job_id == job.id,
            GoodreadsImportJobRow.user_id == user_id,
            GoodreadsImportJobRow.result.in_(["failed", "skipped"]),
        )
        .order_by(GoodreadsImportJobRow.row_number.asc())
        .limit(preview_limit)
    ).scalars()
...
def _record_row(
    ...
    *,
    ...
    existing = session.scalar(
        sa.select(GoodreadsImportJobRow).where(
            GoodreadsImportJobRow.job_id == job.id,
            GoodreadsImportJobRow.identity_hash == row.identity_hash,
        )
    )
    if existing is None:
        entry = GoodreadsImportJobRow(
            job_id=job.id,
            user_id=job.user_id,
            row_number=row.row_number,
...
def _record_issue(
    ...
    *,
    job: GoodreadsImportJob,
    issue: GoodreadsParseIssue,
    result: str = "failed",
) -> None:
    existing = session.scalar(
        sa.select(GoodreadsImportJobRow).where(
            GoodreadsImportJobRow.job_id == job.id,
            GoodreadsImportJobRow.identity_hash == issue.identity_hash,
        )
    )
    if existing is None:
        session.add(
...
def _manual_external_provider_id(identity_hash: str) -> str:
    return f"goodreads:{identity_hash}"
...
def _resolve_or_create_manual_work(
    ...
    *,
    ...
    provider_id = _manual_external_provider_id(row.book_id or row.identity_hash)
    existing_external = session.scalar(
        sa.select(ExternalId).where(
            ExternalId.entity_type == "work",
            ExternalId.provider == "goodreads_manual",
...
def _pick_session_bounds(  # pragma: no cover
    ...
    start = row.date_read or row.date_added
    end = row.date_read or start
    if start is None:
        return None
...
def _mark_job_failed_fresh_session(
    *,
    ...
    try:
        job = get_goodreads_job(terminal_session, user_id=user_id, job_id=job_id)
        job.status = "failed"
...
async def process_goodreads_import_job(
    *,
    user_id: UUID,
    job_id: UUID,
    csv_bytes: bytes,
    ...
    try:
        ...
        parsed = parse_goodreads_csv(
            ...
            shelf_overrides=shelf_overrides,
        )
        ...
        for issue in parsed.issues:
            ...
            if issue_code is None and issue.message.startswith(
                "unsupported read status"
            ):
...
def serialize_job_rows(rows: list[GoodreadsImportJobRow]) -> list[dict[str, Any]]:
