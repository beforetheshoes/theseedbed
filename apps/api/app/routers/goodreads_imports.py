...
from app.db.models.imports import GoodreadsImportJob
from app.db.session import get_db_session
from app.services.goodreads_imports import (
    create_goodreads_job,
    get_active_goodreads_job,
    get_goodreads_job,
    list_goodreads_job_rows,
    process_goodreads_import_job,
    serialize_job,
    serialize_job_rows,
)
...
router = APIRouter(
    prefix="/api/v1/imports/goodreads",
    tags=["goodreads-imports"],
...
def _mark_job_failed_if_stale(session: Session, *, job: GoodreadsImportJob) -> bool:
...
async def create_import_job(
    ...
    shelf_overrides: Annotated[str | None, Form()] = None,
    skipped_rows: Annotated[str | None, Form()] = None,
    skip_reasons: Annotated[str | None, Form()] = None,
) -> dict[str, object]:
    filename = (file.filename or "goodreads.csv").strip()
    ...
    parsed_shelf_overrides = _parse_row_override_payload(
        shelf_overrides, key_name="shelf_overrides"
    )
    ...
    active_job = get_active_goodreads_job(session, user_id=auth.user_id)
    ...
    job = create_goodreads_job(session, user_id=auth.user_id, filename=filename)
    background_tasks.add_task(
        ...
        shelf_overrides=parsed_shelf_overrides,
...
def get_import_job(
    ...
    try:
        job = get_goodreads_job(session, user_id=auth.user_id, job_id=job_id)
...
def get_import_job_rows(
    ...
    try:
        rows, next_cursor = list_goodreads_job_rows(
            session,
            user_id=auth.user_id,
            job_id=job_id,
            limit=limit,
