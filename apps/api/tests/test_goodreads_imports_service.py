...
from app.services.goodreads_parser import GoodreadsParseIssue
...
def test_manual_external_provider_id_prefix() -> None:
    assert _manual_external_provider_id("xyz") == "goodreads:xyz"
...
def test_get_active_goodreads_job_returns_scalar_result() -> None:
    marker = object()
    session = _FakeScalarSession(marker)
    result = get_active_goodreads_job(
        cast(Any, session),
...
def test_mark_job_failed_fresh_session_updates_terminal_fields(
    ...
    monkeypatch.setattr(
        "app.services.goodreads_imports.create_db_session",
        lambda: terminal_session,
    )
    monkeypatch.setattr(
        "app.services.goodreads_imports.get_goodreads_job",
        lambda *_args, **_kwargs: stale_job,
    )
    monkeypatch.setattr(
        "app.services.goodreads_imports.dt",
        SimpleNamespace(
