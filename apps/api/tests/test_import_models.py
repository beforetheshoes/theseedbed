...
def test_goodreads_import_job_model_shape() -> None:
    table = GoodreadsImportJob.__table__
    assert set(table.columns.keys()) == {
        "id",
        "user_id",
        "filename",
    ...
    status_type = cast(sa.Enum, table.columns["status"].type)
    assert status_type.name == "goodreads_import_job_status"


def test_goodreads_import_row_model_shape() -> None:
    table = GoodreadsImportJobRow.__table__
    assert set(table.columns.keys()) == {
        "id",
        "job_id",
        "user_id",
    ...
    result_type = cast(sa.Enum, table.columns["result"].type)
    assert result_type.name == "goodreads_import_row_result"
